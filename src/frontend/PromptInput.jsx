...
const PromptInput = () => {
  ...
  const send = async (e) => {
    ...
    const { message } = choices[0];
		...
		dispatch({
			type: "add_qna",
			payload: { question: state.currentQuestion, answer: message.content },
		});
	...
	return (
		<Frame>
			<form className="flex flex-row w-full items-center" onSubmit={send}>
				<div className="flex flex-row bg-[#ECECF1] dark:bg-light-grey items-center justify-start w-full p-2 mr-6 rounded-md">
					<img
					...
					<input
						type="text"
						className="text-dark-grey dark:text-white text-sm pl-2 bg-transparent outline-none w-full"
					...
					{loading ? (
						<svg
							aria-hidden="true"
							class="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-500 fill-gray-400"
							viewBox="0 0 100 101"
							fill="none"
